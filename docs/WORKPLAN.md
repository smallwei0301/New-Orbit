...
| # | Issue | 摘要 |
|---|---|---|
| M0-1 | CI：GitHub Actions 跑 build + smoke | PR 自動驗證，紅的擋 merge |

### M1 — 顧客端接真實 API（產品能被「客人」用起來）
| # | Issue | 摘要 |
|---|---|---|
| M1-1 | 顧客端店面首頁與商品詳情接真實 API | StoreHome / ItemDetail 換掉 mock |
| M1-2 | 預約流程 BookingFlow 接 API（時段可用性 + 公開建立預約） | 核心轉換路徑 |
| M1-3 | 我的預約 / 揪團 / 付款結果接 API | MyBookings / ClaimBooking / PaymentResult |
| M1-4 | 商家註冊與密碼重設後端實作 | auth/sign-up、forgot/reset-password |

### M2 — 第三方整合（自動化賣點成真）
| # | Issue | 摘要 |
|---|---|---|
| M2-1 | LINE 整合：真實測試連線與 webhook | /v2/bot/info、webhook 簽章驗證 |
| M2-2 | 金流串接：ECPay / 街口 | 付款建立、回調、payment_status 流轉 |
| M2-3 | 通知系統：排程提醒與 quota | Email 先行、模板變數、扣抵 |

### M3 — 營運強化（上線前最後一哩）
| # | Issue | 摘要 |
|---|---|---|
| M3-1 | 安全與營運強化清單 | rate limit、輸入驗證、錯誤處理、監控 |
